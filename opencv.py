...
#TARGET_LAT = 6.096826 # Target latitude
#TARGET_LON = 2.085096  # Target longitude
#TARGET_ALT = 3.927689 # Target altitude in meters'
...
async def main():
    ...
    print("-- Moving to target location based on latitude, longitude, and altitude")
    ...
    while True:
        async for position in drone.telemetry.position():
            ...
            # Check if the drone is within a threshold of the target location
            if (abs(current_lat - TARGET_LAT) < 0.00001 and 
                abs(current_lon - TARGET_LON) < 0.00001 and 
                abs(current_alt - TARGET_ALT) < 0.5):
        ...
        if video.frame_available():
            ...
            if bbox is not None:
                print(bbox)
                target_longitude = 2.085096  # Replace with your target longitude
                target_latitude = 6.096826      # Replace with your target latitude
        
                # Retrieve current drone position (longitude, latitude)
                current_longitude = drone.position.longitude  # Assuming you have a way to get the drone's longitude
                current_latitude = drone.position.latitude    # Assuming you have a way to get the drone's latitude
        
                # Calculate differences
                longitude_diff = target_longitude - current_longitude
                latitude_diff = target_latitude - current_latitude
                
                # Set the velocity based on the differences (scale as necessary)
                velocity = [longitude_diff * scaling_factor, latitude_diff * scaling_factor, 0.1, 0.0]  # Adjust Z velocity for upward movement
